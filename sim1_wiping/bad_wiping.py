...
if __name__ == "__main__":
    ...
    n_CBF = 1
    cbf_qp = init_proxsuite_qp(n_v=3, n_eq=0, n_in=n_CBF)
    ...
    desired_controls = np.zeros([horizon, 3], dtype=config.np_dtype)
    safe_controls = np.zeros([horizon, 3], dtype=config.np_dtype)
    ...
    for i in range(horizon):
        ...
        if i*dt >= circle_start_time:
            ...
            traj_2d = np.array([traj[index,0], traj[index,1], 0], dtype=config.np_dtype)
            states_dt = np.array([v_EE[0], v_EE[1], v_EE[5]], dtype=config.np_dtype)
            traj_2d_dt = np.array([traj_dt[index,0], traj_dt[index,1], 0], dtype=config.np_dtype)
            traj_2d_dtdt = np.array([traj_dtdt[index,0], traj_dtdt[index,1], 0], dtype=config.np_dtype)
            Kp_2d = np.diag([10,10,10]).astype(config.np_dtype)
            Kd_2d = np.diag([10,10,10]).astype(config.np_dtype)
            u_2d_nominal = traj_2d_dtdt - Kp_2d @ (states-traj_2d) - Kd_2d @ (states_dt-traj_2d_dt)

            time_diff_helper_tmp = 0
            if (CBF_config["active"]):
                # Matrices for the CBF-QP constraints
                C = np.zeros([n_CBF, 3], dtype=config.np_dtype)
            ...
            u_2d = u_2d - dJdq_EE[[0,1,5]]
            S_primary = (J_EE @ Minv)[[0,1,5],:]

            # Secondary Objective: track the trajectory in 3D
            Kp_task = np.diag([40,40,40,100,100,100]).astype(config.np_dtype)
            Kd_task = np.diag([30,30,30,50,50,75]).astype(config.np_dtype)
            ...
            R_d = np.array([[1, 0, 0],
                            ...
                            [0, 0, -1]], dtype=config.np_dtype)
            index = int(np.round(dt*i/Ts))
            S_secondary, u_3d = get_torque_to_track_traj_const_ori(traj[index,:], traj_dt[index,:], traj_dtdt[index,:], R_d,
                                                        Kp_task, Kd_task, Minv, J_EE, dJdq_EE, dq, P_EE, R_EE)
            ...
            # Compute the input torque
            # S_primary_pinv = S_primary.T @ np.linalg.pinv(S_primary @ S_primary.T + 0.01* np.eye(S_primary.shape[0]))
            S_primary_pinv = np.linalg.pinv(S_primary)
            S_primary_null = np.eye(n_joints) - S_primary_pinv @ S_primary
            # S_secondary_pinv = S_secondary.T @ np.linalg.pinv(S_secondary @ S_secondary.T + 0.01* np.eye(S_secondary.shape[0]))
            S_secondary_pinv = np.linalg.pinv(S_secondary)
            S_secondary_null = np.eye(n_joints) - S_secondary_pinv @ S_secondary

            u = u_press + u_friction + nle
            # u = nle
            u += S_primary_pinv @ u_2d 
            u += S_primary_null @ (S_secondary_pinv @ u_3d + S_secondary_null @ u_joint)
