...
if __name__ == '__main__':
    ...
    dq_lb = np.array([-2.1750, -2.0, -2.1750, -2.1750, -2.6100, -2.6100, -2.6100])
    dq_ub = np.array([2.1750, 2.0, 2.1750, 2.1750, 2.6100, 2.6100, 2.6100])
    ...
    n_in = n_controls + n_CBF + n_controls 
    ...
    for i in range(len(selected_BBs)):
        ...
        for j in range(len(obstacle_SFs)):
            ...
            probs.addProblem(prob, rob_frame_id, 0)
    ...
    # Pick the future position and velocity of the ball
    pick_idx = 2
    ...
    try:
        ...
        while time.time() - t_start < 60:
            ...
            all_P_obs_np[0, :] = ball_pos_in_base
            all_quat_obs_np[0, :] = np.array([0., 0., 0., 1.])
            all_v_obs_np[0, :] = ball_vel_in_base
            all_omega_obs_np[0, :] = np.zeros(3)
            all_v_dot_obs_np[0, :] = ball_acc_in_base
            all_omega_dot_obs_np[0, :] = np.zeros(3)
            ...
            C[n_CBF+n_controls:n_CBF+2*n_controls,:] = np.eye(n_controls)
